...
type UploadsInterface interface {
	...
	GetProfileImage(repositoryPath, fileName, fileExtension string, userId uuid.UUID) (models.GetImage, error)
	UpdateProfilePicture(repositoryPath, oldFileName, oldFileExtension string, NewImage models.UploadImageForm) (models.ImageData, error)
	CreateCollection(userId uuid.UUID, collectionName, repositoryPath string) (models.CollectionData, error)
	UploadImageIntoCollection(collectionPath string, form models.UploadImageForm) (models.ImageData, error)
